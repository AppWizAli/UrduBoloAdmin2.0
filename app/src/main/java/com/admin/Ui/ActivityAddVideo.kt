...
class ActivityAddVideo : AppCompatActivity(), AdapteraddVideo.OnItemClickListener {
    ...
    private fun showDialogAddVideo() {
        ...
        val videlink = dialog.findViewById<EditText>(R.id.videolink)
        val radioGroup = dialog.findViewById<RadioGroup>(R.id.radioGroupPrivacy)
        val radioGroupDownloadAccess =
            dialog.findViewById<RadioGroup>(R.id.radioGroupDownloadAccess)
        ...
        next.setOnClickListener {
            ...
            newModelVideo.videourl=videlink.text.toString()
            ...
            when (selectedRadioButtonId) {
                ...
                else -> Toast.makeText(mContext, "Please select video privacy", Toast.LENGTH_SHORT)
                    .show()
            }
    ...
    private fun setAdapter() {
        binding.rvAddedVideos.layoutManager =
    ...
    private fun uploadNextVideo() {
        ...
        if (currentVideoIndex == listVideos.size) {
            Toast.makeText(mContext, "All Episodes Uploaded Successfully!!!", Toast.LENGTH_SHORT)
                .show()
        ...
        if (currentVideoIndex < listVideos.size) {
            ...
            uploadThumbnailImage(modelVideo.thumbnail.toUri()) { thumbnailUrl ->
                if (thumbnailUrl != null) {
                    ...
                    Toast.makeText(
                        mContext,
                        "Failed to upload thumbnail for video ${currentVideoIndex + 1}",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
...
/*
    fun convertToDirectDownloadLink(shareLink: String): String {
        val fileIdIndex = shareLink.indexOf("/d/") + 3 // Find the index where FILE_ID starts
        val fileId = shareLink.substring(fileIdIndex, shareLink.indexOf("/view", fileIdIndex)) // Extract FILE_ID
